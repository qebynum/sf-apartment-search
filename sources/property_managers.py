...
def scrape_rentalsinsf(max_price, min_sqft, neighborhoods):
    ...
    for url in RENTALSINSF_CATEGORY_URLS:
        ...
        for a in detail_links:
            ...
            # Lightweight short-term check (keyword only — this site's
            # listings don't carry the free-text "available through <date>"
            # phrasing Craigslist posts do, so no date parsing needed here).
            SHORT_TERM_KEYWORDS = ["month to month", "month-to-month", "short term", "short-term", "temporary"]
            if any(k in block_text.lower() for k in SHORT_TERM_KEYWORDS):
                continue
